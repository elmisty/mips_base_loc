...
function pppoeReconnectModeConvert($mode)
{
	if ($mode == "alwayson") { return "AlwaysOn"; }
...
else if (query($waninetp."/addrtype")=="ipv6")
{
	...
	if ($str_wantype == "6IN4")
	{
		$rwan4 = XNODE_getpathbytarget("/runtime", "inf", "uid", $WAN3, "0");
		...
		$child_infp = XNODE_getpathbytarget("", "inf", "uid", $WAN3, "0");
	...
	else if ($str_wantype == "6RD")
	{
		...
		$IPv6_6rd_assigned_prefix = get("",$rwan4."/blackhole/entry:1/prefix")."/".get("",$rwan4."/blackhole/entry:1/plen");
	}
...
else if ($is_ppp10 == 1)
{
	...
	$rwan  = XNODE_getpathbytarget("/runtime", "inf", "uid", $WAN1, "0");    // Original Code.
	$rwan4 = XNODE_getpathbytarget("/runtime", "inf", "uid", $WAN4, "0");

	// WAN-3 is child of WAN-1, in Share with IPv4.
	$child_uid  = query($rwan."/child/uid");
	$rwan_child = XNODE_getpathbytarget("/runtime", "inf", "uid", $child_uid, "0");
	...
	*/
	...
	$ppp_infp  = XNODE_getpathbytarget("", "inf", "uid", $WAN1, "0");
...
{
	if ($str_wantype == "6IN4") {
		$StatusV6_wanDNSserver   = query($rwan3."/inet/ipv6/dns:1");
		$StatusV6_wanDNSserver2  = query($rwan3."/inet/ipv6/dns:2");
	} else {
...
else if($str_wanDNSserver != "" && $str_wanDNSserver2 == "")
{
	if($str_wantype == "PPPoE")
	{
		...
		if ($StatusV6_wanDNSserver == $StatusV6_wanDNSserver2)
...
{
/*	HuanYao: No idea why do we change wantype when wan cable is not connected.
...
if($DEBUG_HNAP == "y")
{
	TRACE_info("=== [ GetIPv6Status.php ] DEBUG msgs ===");

	TRACE_info("  ++ $StatusV6_wanipaddr    = ".$StatusV6_wanipaddr);
	TRACE_info("  ++ $StatusV6_DefaultGateway  = ".$StatusV6_DefaultGateway);
	TRACE_info("  ++ $StatusV6_wanDNSserver = ".$StatusV6_wanDNSserver);
	TRACE_info("  ++ $StatusV6_wanDNSserver2 = ".$StatusV6_wanDNSserver2);
}
...
if($DEBUG_HNAP == "y")
{
	if($lan=="")	  TRACE_info("$lan is empty.");
	if($rlan=="")	  TRACE_info("$rlan is empty.");
	if($inetuid=="")  TRACE_info("$inetuid is empty.");
	if($phyuid=="")	  TRACE_info("$phyuid is empty.");
	if($rlanphyp=="") TRACE_info("$rlanphyp is empty.");
}
...
$rwan4 = XNODE_getpathbytarget("/runtime", "inf", "uid", $WAN4, "0");
if ($str_wantype == "6IN4") {
	$pdnetwork = query($rwan3."/child/pdnetwork");
	$pdprefix  = query($rwan3."/child/pdprefix");
} else {
...
else if($str_wantype == "PPPoE")
{
	...
	else if(query($wan1inetp."/addrtype")=="ppp10")
	{
		...
		$PppoeReconnectMode = pppoeReconnectModeConvert(query($wan1inetp."/ppp6/dialup/mode"));
	...
	else //ppp6
	{
		...
		$PppoeReconnectMode = pppoeReconnectModeConvert(query($wan3inetp."/ppp6/dialup/mode"));
...
if($ConnectionType != "IPv6_LinkLocalOnly")
{
	...
	else if($ConnectionType=="IPv6_AutoDetection" && $ConnectionType2!="IPv6_6RD")
	{
		$pppcnt = query($waninetp."/ppp6/dns/count");
		$ipv6cnt = query($waninetp."/ipv6/dns/count");

		if($pppcnt != "0" && $ipv6cnt!= "0") $ObtainDNS = "Manual";
		else $ObtainDNS = "Automatic";
	}
