...
function phyinf_setipv6($layout, $ifname)
{
	...
	startcmd('event IPV6ENABLE insert "'.$ifname.':echo 0 > /proc/sys/net/ipv6/conf/'.$phy.'/disable_ipv6"');
	stopcmd('event IPV6ENABLE remove "'.$ifname.'"');
	// HuanYao: Enable IPv6 imminently to solve ENLAN timing issue.
	// HuanYao (2015.06.18): Do not enable IPv6 here. Enable this in ENLAN service.
