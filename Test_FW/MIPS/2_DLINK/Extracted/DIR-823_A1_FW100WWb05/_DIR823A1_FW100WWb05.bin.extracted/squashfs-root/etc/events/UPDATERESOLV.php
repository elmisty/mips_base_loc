...
$lan_ip = query($path_inet_lan1."/ipv4/ipaddr");
$lan_mask = query($path_inet_lan1."/ipv4/mask");
...
foreach ("/runtime/inf")
{
	...
	$lower_infp = "";
	$ppp_server_ip = "";
	//fix the DNS server in LAN can't access internet issue,
	//prevent the routing rule to gateway when DNS server in LAN
	$dns_in_lan =0;
	...
	if ($addrtype=="ipv4" || $addrtype=="ppp4")
	{
		...
		//	fwrite("a", $START, "#def: ".$def."\n");
			...
			{
				$lan_netid	= ipv4networkid($lan_ip, $lan_mask);
				$dns_netid	= ipv4networkid($VaLuE, $lan_mask);
				if($lan_netid == $dns_netid)	$dns_in_lan = 1;
				else													$dns_in_lan = 0;			
