...
if ($enable==1 && $enablev6==1)
{
	...
	else
	{
		...
		fwrite(a, $ntp_run,
			...
			'	ntpclient -h $SERVER4 -i 5 -s -4 || ntpclient -h $SERVER4 -i 5 -s -6 > /dev/console\n'.	/* HuanYao: for user-configured ntp server, try use ipv6 when ipv4 failed. */
			'	if [ $? != 0 ]; then\n'.
			'		xmldbc -k ntp\n'.
...
else if ($enable==1 && $enablev6==0)
{
	...
	else
	{
		...
		fwrite(a, $ntp_run,
			...
			'ntpclient -h '.$server.' -i 5 -s -4 || ntpclient -h $SERVER4 -i 5 -s -6 > /dev/console\n'.	/* HuanYao: for user-configured ntp server, try use ipv6 when ipv4 failed. */
			'if [ $? != 0 ]; then\n'.
			'	xmldbc -k ntp\n'.
