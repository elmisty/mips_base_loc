...
function check_mac_address($inf,$check_mac)
{
	$ret =0;
	//TRACE_info("check mac=".$check_mac);
	$path = XNODE_getpathbytarget("/runtime", "phyinf", "uid", $inf, 0);
	foreach($path."/media/clients/entry")
	{
		TRACE_info($mac);
		$mac= query("macaddr");
		if($check_mac == $mac)
...
function is_wifi_client($ipaddr)
{
	$ret = 0;
	$mac_addr = INET_ARP($ipaddr);
	if($mac_addr !="")
	$mac_addr= toupper($mac_addr);

	$ret = check_mac_address("BAND24G-1.2", $mac_addr);
	if($ret == "1") return $ret;


	$ret = check_mac_address("BAND24G-1.1", $mac_addr);
	if($ret == "1") return $ret;

	$ret = check_mac_address("BAND5G-1.1", $mac_addr);
	if($ret == "1") return $ret;
