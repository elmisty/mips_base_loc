...
function is_duplicated($list, $item, $delimiter)
{
	$i = 0;
	$cnt = cut_count($list, $delimiter);
	while ($i < $cnt)
	{
		if ($item == cut($list, $i, $delimiter))
		{ return 1; }
		$i = $i + 1;
	}
	return 0;
...
function commands($inf, $stsp, $phyinf, $dhcpsp)
{
	...
	if($pd_ptlft == "" || $pd_vtlft == "" || $pd_ptlft == "-1" || $pd_vtlft == "-1") 
	{ 
		if ($routerlft == "")
		{
			$pd_ptlft="3600"; 
...
*/			
	...
	stopcmd('if [ "$combuid" != "" ] && [ "$combuid" != "'.$inf.'" ]; then\n'.
