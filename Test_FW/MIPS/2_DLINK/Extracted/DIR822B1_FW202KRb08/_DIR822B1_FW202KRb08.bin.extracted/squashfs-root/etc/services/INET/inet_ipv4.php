...
function inet_ipv4_dynamic($inf, $ifname, $inet, $inetp)
{
    ...
    fwrite(w,$udhcpc_helper,
		'#!/bin/sh\n'.
		'echo [$0]: $1 $interface $ip $subnet $router $lease $domain $scope $winstype $wins $sixrd_prefix $sixrd_prefixlen $sixrd_msklen $sixrd_bripaddr $tr069_acs_url $tr069_provisioning_code... > /dev/console\n'.
		'phpsh '.$hlper.' ACTION=$1'.
			...
			' "TR069ACSURL=$tr069_acs_url"'.
			' "TR069PROVISIONINGCODE=$tr069_provisioning_code"'.
			' "SDEST=$sdest"'.
			' "SSUBNET=$ssubnet"'.
			' "SROUTER=$srouter"\n'.
		'exit 0\n'
	...
	startcmd(
		...
		'event "'.$inf.'.DHCP.RELEASE" insert "WANLED:phpsh /etc/scripts/update_wanled.php EVENT=WAN_DHCP_RELEASE"\n'.
	...
	if($layout != "bridge")
		startcmd('udhcpc '.$unicast.'-i '.$ifname.' -H '.$hostname.' -p '.$udhcpc_pid.' -s '.$udhcpc_helper.' '.$dhcpplus_cmd.' &\n');
	else{
		startcmd('event WANPORT.LINKUP insert "service INF.BRIDGE-1 restart"\n');
		startcmd('udhcpc '.$unicast.'-i '.$ifname.' -H '.$hostname.' -p '.$udhcpc_pid.' -s '.$udhcpc_helper.' '.$dhcpplus_cmd.' -D 3 -R 3 -S 10 &\n');
		}
