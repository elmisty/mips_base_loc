...
foreach($nodebase) //Prevent the security issue of Command Injection
{
	//Ignore the metric if interface is LAN-PD
	if(get("", "Interface") == "LAN(DHCP-PD)")
	{
		set("Metric", "NaN");
	}

	if(get("", "Gateway") != "" && INET_validv6addr(get("", "Gateway"))==0)
	{
		$result = "ERROR";
		break;
	}
	else if(get("", "DestNetwork") != "" && INET_validv6addr(get("", "DestNetwork"))==0)
	{
		$result = "ERROR";
		break;
	}
	else if(get("", "Metric") != "" && get("", "Metric") != "NaN" && isdigit(get("", "Metric"))==0)
	{
		$result = "ERROR";
		break;
	}
...
if($result=="REBOOT")
{
	...
	fwrite("a",$ShellPath, "service DHCPS6.LAN-4 restart > /dev/console\n");
	/*
