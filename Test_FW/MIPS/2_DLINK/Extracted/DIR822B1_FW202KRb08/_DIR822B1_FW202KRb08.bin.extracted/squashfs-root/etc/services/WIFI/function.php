...
function devname($uid)
{
	...
	else if($uid == "BAND24G-1.3")
		return $_GLOBALS["BAND24G_GUEST_DEVNAME2"];
	else if($uid == "BAND24G-1.4")
		return $_GLOBALS["BAND24G_GUEST_DEVNAME3"];
	...
	else if($uid == "BAND5G-1.3")
		return $_GLOBALS["BAND5G_GUEST_DEVNAME2"];
	else if($uid == "BAND5G-1.4")
		return $_GLOBALS["BAND5G_GUEST_DEVNAME3"];
...
function guestaccess($wifi_uid,$dev)
{
	...
	if($guestaccess == "" || $guestaccess == "DISABLE" || $layout=="bridge")
...
function set_br_shortcut($name,$dev)
{
	...
	if($active == 1)
	{
		...
		if($QOS_ENABLE == 1 && $qostype == 1) //by protocol
			fwrite("a", $_GLOBALS["START"], 'iwpriv '.$dev.' set_mib disable_brsc=1\n');
		else
			fwrite("a", $_GLOBALS["START"], 'iwpriv '.$dev.' set_mib disable_brsc=0\n');
	}
}
?>
