...
function lan_default($infp, $stsp, $name, $dev)
{
	...
	while ($i > 0)
	{
		...
		startcmd("iptables -t nat -A PREROUTING -i ".$dev." -d ".$wip." -j UPNP");
		startcmd("iptables -t nat -A UPNP -j DNAT.UPNP");
		$i++;
	}
	
	/* add for Page Redirect, sammy */
	if($name=="LAN-1") { startcmd("iptables -t nat -A PREROUTING -i ".$dev." -p tcp --dport 80 -d ! ".$ipaddr."/".$mask." -j PRD.LAN-1\n"); }
...
//marco, for cone	
	...
	{
		$ppp4_path = XNODE_getpathbytarget("/inet", "entry", "uid", query("uid"), 0);
		if(query($ppp4_path."/ppp4/dialup/mode")!="ondemand")
			stopcmd("echo 0 > /proc/sys/net/ipv4/ip_forward");
	}
}
