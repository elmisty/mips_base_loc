...
if(query("/device/layout") == "bridge")
{
	if($Interface == "WAN")
	{
		//runtime/phyinf:5/media/wifi/ssid
		$path_wifi_24g = XNODE_getpathbytarget("/runtime", "phyinf", "uid", "BAND24G-REPEATER");
		$client_24g= query($path_wifi_24g."/media/wifi/ssid");
		$path_wifi_5g = XNODE_getpathbytarget("/runtime", "phyinf", "uid", "BAND5G-REPEATER");
		$client_5g= query($path_wifi_5g."/media/wifi/ssid");
		if($client_24g == ""){
			if($client_5g != ""){  //5g
				if (isdigit($tx) == 0)
				{
				...
				$tx += query($path_wifi_5g.$path_tx);
				$rx += query($path_wifi_5g.$path_rx);
				$tx_pkts += query($path_wifi_5g.$path_tx_pkts);
				$rx_pkts += query($path_wifi_5g.$path_rx_pkts);
				$tx_drop += query($path_wifi_5g.$path_tx_drop);
				$rx_drop += query($path_wifi_5g.$path_rx_drop);
				$tx_error = query($path_wifi_5g.$path_tx_error);
				$rx_error = query($path_wifi_5g.$path_rx_error);
				$error += $tx_error+$rx_error;
			}
		}
		else if($client_24g != ""){ //24g
			if (isdigit($tx) == 0)
			{
				$tx = 0;                                $rx = 0;
				$tx_pkts = 0;                       $rx_pkts = 0;
			...
			$tx += query($path_wifi_24g.$path_tx);
			$rx += query($path_wifi_24g.$path_rx);
			$tx_pkts += query($path_wifi_24g.$path_tx_pkts);
			$rx_pkts += query($path_wifi_24g.$path_rx_pkts);
			$tx_drop += query($path_wifi_24g.$path_tx_drop);
			$rx_drop += query($path_wifi_24g.$path_rx_drop);
			$tx_error = query($path_wifi_24g.$path_tx_error);
			$rx_error = query($path_wifi_24g.$path_rx_error);
			$error += $tx_error+$rx_error;
		}
	}
}
