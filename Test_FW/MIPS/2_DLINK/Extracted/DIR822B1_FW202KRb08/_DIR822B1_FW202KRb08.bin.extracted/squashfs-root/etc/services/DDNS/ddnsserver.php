...
function ddns4generate($inf, $phyinf, $ddnsp,$v6inf,$v6enable,$v6phyinf)
{
	...
	if( $provider != "ORAY")
	{
		...
		*/
		...
		if($provider == "DYNDNS.Service"){
			...
			fwrite("a",$_GLOBALS["START"], '#stsp='.$stsp.'\n');
			fwrite("a",$_GLOBALS["START"], '#sn='.$sn.'\n');
			fwrite("a",$_GLOBALS["START"], '#lanmac='.$lanmac.'\n');
			$m0 = cut($lanmac, "0", ":");
			$m1 = cut($lanmac, "1", ":");
			$m2 = cut($lanmac, "2", ":");
			$m3 = cut($lanmac, "3", ":");
			...
			fwrite("a",$_GLOBALS["START"], '#lanmac_ddns='.$lanmac_ddns.'\n');
			...
			fwrite("a",$_GLOBALS["START"], $set_ipupdate.'\n'.
						...
						'xmldbc -s '.$stsp.'/ddns4/status IDLE\n'.
			...
			fwrite("a", $_GLOBALS["STOP"],
					...
					'xmldbc -s '.$stsp.'/ddns4/status DISCONNECTING\n'.
					'exit 0\n');
		...
		if($w0 != "www" && $w1 != "noip" || $w0 != "www" && $w1 != "no-ip"){
	...
	else
	{
		...
		fwrite("a",$_GLOBALS["START"],
				...
				// 'susockc /var/run/ddnsd.susock '.$set.'\n'.
				'echo Start peanut...\n'.
				'peanut -S "5" -n "'.$hostname.'" -u "'.$username.'" -p "'.$password.'" -i '.$ipaddr.' -q "/var/run/peanut.info" > /dev/console &\n'.
				'xmldbc -s '.$stsp.'/ddns4/valid 1\n'.
				'xmldbc -s '.$stsp.'/ddns4/provider '.$provider.'\n'.
				'exit 0\n');
