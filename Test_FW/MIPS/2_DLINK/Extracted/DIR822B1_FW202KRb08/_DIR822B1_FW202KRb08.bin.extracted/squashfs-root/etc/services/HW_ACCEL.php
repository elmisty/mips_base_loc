...
function HWNATsetup($dis_fastnat,$dis_alpha_sw_nat,$wan_type)
{
...
if ($layout=="router")
{
	...
	*/
		...
		if ($bwc_profile_name != "")
		{
			...
			if ($bwcp != "")
			{
				if( query($bwcp."/enable") == "1")
				{
					...
					if($bwcp_flag == "TC_WFQ" || $bwcp_flag == "TC_SPQ" || $bwcp_flag == "TC_SPQ_2013GUI" ) {$dis_fastnat_flag = 1; $dis_alpha_nat_flag = 1;}
				}
			}
		}
	}
	...
	//Turn off the NAT if the Super-DMZ is enabled.
	if(get("", "/nat/entry/sdmz/enable")=="1") 
	{
		$dis_alpha_nat_flag=1;
	}


	/* +++ START Hardware NAT and Fast NAT +++ */
	if($wan1_active == "1" || $wan2_active == "1") {HWNATsetup($dis_fastnat_flag,$dis_alpha_nat_flag,$wan_mode);}
}
