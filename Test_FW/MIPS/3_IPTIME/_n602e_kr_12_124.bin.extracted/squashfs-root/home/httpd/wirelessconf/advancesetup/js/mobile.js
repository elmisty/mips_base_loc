...
function make_local_postdata()
{
	...
	if(!config_data.use_sta_wds && $('[sid=\"C_ADVANCESETUP_MODE\"]').val() == 'wds'){
		localdata.push({'name':'mac', 'value' : make_macstr()});
	}
	
	if($('[sid=\"C_ADVANCESETUP_MODE\"]').val() == "nouse" || $('[sid=\"C_ADVANCESETUP_SSID\"]').val() !== current_info.ssid)
		current_info.ssid_hex = null;
	if(current_info.ssid_hex)
		localdata.push({'name':'ssid_hex', 'value' : current_info.ssid_hex});
	return localdata;
}
...
function advancesetup_validate()
{
	...
	if(val == 'nouse'){
	...
	else if(!config_data.use_sta_wds && val == 'wds'){
		...
		if(val == 'wwan'){
			if(config_data.otherwwan == '1'){
				alert(M_lang['S_OTHERWWAN_ALERT1'] + M_lang['S_OTHERWWAN_ALERT2']);
...
function pageview_select_update()
{
	...
	if(config_data.support_wl_list.length > 1){
		$('[sid="PAGE_MENU"]').remove();
		$('[sid="S_PAGE_TITLE"]').parent().prepend('<select sid=\"PAGE_MENU\"></select>');
		//$('[sid="S_PAGE_TITLE"]').text("");
		//$('[sid="S_PAGE_TITLE"]').after('<p sid="S_PAGE_TITLE2" id="ptitle">'+M_lang["S_PAGE_TITLE2"]+'</p>');
		$('[sid="S_PAGE_TITLE"]').after('<span class="left_span_gg">'+M_lang["S_PAGE_TITLE1"]+'</span><span class="right_span">'+M_lang["S_PAGE_TITLE2"]+'</span>');
		//$('[sid="S_PAGE_TITLE"]').text(M_lang["S_PAGE_TITLE"]+M_lang["S_PAGE_TITLE2"]);		

		for(var i = 0; i < config_data.support_wl_list.length; i++){
			$('[sid="PAGE_MENU"]').append('<option value=\"'+config_data.support_wl_list[i].tag+'\" ' +(i == 0?'selected':'')+ '>'+
				(config_data.support_wl_list[i].text) +'</option>');
		}
		//$('[sid="PAGE_MENU"]').append('<option value=\"5g\" selected>5GHz</option>');
		//$('[sid="PAGE_MENU"]').append('<option value=\"2g\">2.4GHz</option>');
		$('[sid="PAGE_MENU"]').css('display','inline-block').css('float','left').css('margin-left','1em').css('width','7em');
	...
	if(data_local){
		...
		if(dataObj){
			...
			if(!config_data.use_sta_wds && $('[sid=\"C_ADVANCESETUP_MODE\"]').val() == 'wds') {
				...
				current_info.ssid = dataObj.essid;
				current_info.ssid_hex = dataObj.ssid_hex;
				var authval = 'nouse';
	...
	if(statusobj){
		...
		if(identifier == 'C'){
			...
			if(config_data.advancesetup.mode == 'nouse'){
				...
				if(!config_data.use_sta_wds && config_data.advancesetup.mode == 'wds'){
	...
	if(data_local != null){
		...
		$('li>a').on("mousedown touchstart", function() {
			...
			.on("webkitAnimationEnd oanimationend msAnimationEnd animationend", function() {
                ...
                $("li>a:odd").css("background-color","#F9F9F9");
...
$(document).ready(function() {
	...
	events.on('config.update.done', function() {
		current_info.ssid = config_data.advancesetup.ssid;
		current_info.ssid_hex = config_data.advancesetup.ssid_hex;
	});
	/*error_code_local = parseInt(config_data.errcode);
