...
function wl_tag_to_full_ghz_text(wltag)
{
        ...
        for(var i = 0; i < support_wl_modes.length; i++){
                if(wltag == support_wl_modes[i].tag)    return support_wl_modes[i].text;
        }
...
{
	$('.ui-page').css('background-color', '#FAFAF7');
	get_status_local();
}
...
{
	var retstr = '';
	retstr += wl_tag_to_full_ghz_text(cobj.wltag);
...
function make_blank_macline()
{
	...
	HTMLStr += ('<li sid=\"BLANK_MACLINE\" class=\"lc_selmac_li\"><div class=\"lc_selmac\" style=\"padding-left:.9em;\">');
...
function update_connected_macline(cobj)
{
	...
	if(cobj.statstr != '')
		$('#'+idval+' [sid=\"STATUS\"]').text(cobj.statstr + '/' + timestr + ((cobj.wltag)?('/' + wl_tag_to_full_ghz_text(cobj.wltag)):''));
	else
		$('#'+idval+' [sid=\"STATUS\"]').text(timestr + ((cobj.wltag)?('/' + wl_tag_to_full_ghz_text(cobj.wltag)):''));
					...
					for(var i = 0; i < status_data.stalist.length; i++){
						...
						if(!find_newdata_from_old(status_data.stalist[i].mac)){
							if(i == 0){
								...
								$(make_connected_macline(status_data.stalist[i]))
									.insertAfter('#'+status_data.stalist[i-1].mac);
							}
						}
						update_connected_macline(status_data.stalist[i]);
					}
...
iux_update_local_func['selbsslist'] = function()
{
	...
	$("[sid=\"L_SELBSSLIST\"] li:odd").css("background-color","#FFFFFF");
...
iux_update_local_func['delmac'] = function()
{
	...
	for(var i = 0; i < delmac_data.reglist.length; i++){
		...
		$("[sid=\"L_DELMACLIST\"] li:odd").addClass('lc_whitebox');
