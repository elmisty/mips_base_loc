...
submit_local_func['ipdisk'] = function(localpostdata)
{
	if(localpostdata)
	{
		...
		{
			//add* 0130
			$(".content_field").children(":eq(1)").css('width', '22em');
...
submit_local_func['ftp'] = function(localpostdata)
{
	...
	else
	{
		...
		if( !check_input_error("C_FTP_PORT", regExp_port) && !check_input_error("C_FTP_IPDISKPORT", regExp_port)
			 && $('[sid="C_FTP_IPDISKPORT"]').val() != '80' && $('[sid="C_FTP_PORT"]').val() != '80')
			result1 = true;
		else{
			if($('[sid="C_FTP_IPDISKPORT"]').val() == '80' || $('[sid="C_FTP_PORT"]').val() == '80')
				alert(M_lang['S_PORT_80_ERROR']);
		}
		for(var i=0; i<MAX_USER; i ++)
		{
...
function sbutton_change_event()
{
	status_control_enable(!!check_change_value(), "S_BUTTON_SUBMIT");
}
/*----------samba start func----------*/
iux_update_local_func['samba'] = function(id, bvalue)
{
	...
	{
		case 'C':
			var slider_trash = $('[sid="S_SAMBA_NETWORK_TRASH"]');
			slider_trash.val(config_data.samba.network_trash?1:0);
			slider_trash.unbind('change');
			sliderButtonEvent( { object: slider_trash, runFunc: function(){
				status_control_enable(!!check_change_value(), "S_BUTTON_SUBMIT");
			}});
			if(config_data.samba.allow_smb === undefined)
			{
				$("#allow_smb_row").remove();
			}
			else
			{
				$('[sid="S_SAMBA_ALLOW_SMB"]').val(config_data.samba.allow_smb).selectmenu("refresh")
					.unbind('change', sbutton_change_event).change(sbutton_change_event);
			}
...
{
	var obj;
	obj = $("[sid=S_SAMBA_NETWORK_TRASH]");
	if(obj.length !== 0 && obj.val() != config_data.samba.network_trash)
		return true;
	obj = $("[sid=S_SAMBA_ALLOW_SMB]");
	if(obj.length !== 0 && obj.val() != config_data.samba.allow_smb)
		return true;
	...
	if(addoption == "run")
	{
		$("[sid^='C_']").each(function()
		{
			if(($(this).attr("type") == "select") && ($(this).attr('name').indexOf('userproperty') != -1))
			{
				...
				var name = $(this).attr("name");
				var nIndex = name.substring(name.length - 1); 
				if($("[sid=C_SAMBA_RUN]:checked").val() == "0")
					enable = 0;
				else
				$('option:selected',this).val() == "off" ? enable = false : enable = true;
		...
		status_control_enable(bvalue === "1", "S_SAMBA_NETWORK_TRASH");
		status_control_enable(bvalue === "1", "S_SAMBA_ALLOW_SMB");
	}
	else
	{
		status_control_enable(config_data.samba.run === "1", "S_SAMBA_NETWORK_TRASH");
		status_control_enable(config_data.samba.run === "1", "S_SAMBA_ALLOW_SMB");
		for (var i = 0;i < 5 ;i++ )
		{
...
/*----------iptimecloud start func----------*/
iux_update_local_func['iptimecloud'] = function(id, bvalue)
{
	switch(id)
	{
		case 'C':
			config_data.mount.hddlist.forEach(function(obj) {
				if(obj.text.length == 0 || obj.value.length == 0)
					return;
				$('[sid="C_IPTIMECLOUD_HDDNAME"]').append($('<option>', { value: obj.value, text:obj.text}));
			});
			$("[sid=C_IPTIMECLOUD_HDDNAME]").val(config_data.iptimecloud.hddname).selectmenu("refresh");

			status_control_checkbox(bvalue ,"C_IPTIMECLOUD_PORT");
			status_control_enable(bvalue, "PASSWD_VISIBLE");
			insertDefaultOption( 'iptimecloud' )
			break;
		default:
			if(config_data.iptimecloud.datafolder && config_data.iptimecloud.datafolder.length != 0)
			{
				var s_datafolder = config_data.iptimecloud.datafolder.split("/");							
				var Statustxt = "http://" + config_data.iptimecloud.hostname + ":" + config_data.iptimecloud.port + "/" + s_datafolder[1];
			}
			return Statustxt;
	}
};

add_listener_local_func['iptimecloud'] = function(addoption, bvalue)
{
	var dvalue;
	if(addoption == "run")
	{
		status_control_enable(bvalue, "PASSWD_VISIBLE");	
		$('[name="defaultport"]:checked').val() == 1 ? dvalue = false : dvalue = true;
			status_control_enable(dvalue, "C_IPTIMECLOUD_PORT");
	} 
	else
	{
		ctrlmgr_defaultvalue("C_IPTIMECLOUD_PORT");
		ctrlmgr_password_visible("PASSWD_VISIBLE", "C_IPTIMECLOUD_PASSWD");
	}
};

submit_local_func['iptimecloud'] = function(localpostdata)
{
	if(localpostdata)
		{ return true; }
	else
	{
		if($("[sid=C_IPTIMECLOUD_HDDNAME]").val() == "")
		{
			alert(M_lang["S_DATAFOLDER_ERROR"]);
			return false;
		}
		if( !check_input_error("C_IPTIMECLOUD_USERID", regExp_text)
			&& !check_input_error("C_IPTIMECLOUD_PORT", regExp_port))
		{
			return true;
		}
	}
...
function CheckEntwareFolder()
{
	if(config_data.entware.def_admin == '1'){
		alert(M_lang["S_ENTWARE_NOADMIN"]);
		return false;
	}
	return true;
}

function UpdateEntwareUI()
{
	if(!status_data.entware || !config_data.entware)	return;
	
	var selectedval = $('[sid=\"C_ENTWARE_DATAFOLDER\"] option:selected').val();
	if(typeof selectedval === "undefined")	return;

	status_control_enable(true, "C_ENTWARE_DATAFOLDER");
	switch(status_data.entware.install_stat){
		case 'INSTALLING':
			status_control_enable(false, "C_ENTWARE_DATAFOLDER");
		case 'ERROR':
		case 'READY':
		case 'NONE':
		default:
			$('[sid=\"L_ENTWARE_pSTATUS\"]').text(M_lang['S_ENTWARE_STATUS_'+status_data.entware.install_stat]);
			break;
	}

	if(selectedval.indexOf("/") == -1){
		status_control_visible(true, "L_ENTWARE_DATAFOLDER" );
		$('[sid=\"L_TELNET_ONOFF_LINE\"]').hide();
		$('[sid=\"S_BUTTON_ENTWARE_SUBMIT\"]').hide();
		$('[sid=\"S_BUTTON_ENTWARE_INSTALL\"]').show();
		$('[sid=\"L_ENTWARE_STATUS\"]').text(M_lang['S_ENTWARE_STATUS_NONE']);
	}else{
		status_control_visible(false, "L_ENTWARE_DATAFOLDER" );
	}
	if(!selectedval){
		status_control_visible(false, "L_ENTWARE_DATAFOLDER" );
		$('[sid=\"L_TELNET_ONOFF_LINE\"]').hide();
		$('[sid=\"S_BUTTON_ENTWARE_SUBMIT\"]').hide();
		$('[sid=\"S_BUTTON_ENTWARE_INSTALL\"]').hide();
		$('[sid=\"L_ENTWARE_STATUS\"]').text(M_lang['S_ENTWARE_STATUS_NONE']);
	}else if(selectedval == 'clear'){
		status_control_visible(false, "L_ENTWARE_DATAFOLDER" );
		$('[sid=\"L_TELNET_ONOFF_LINE\"]').hide();
		$('[sid=\"S_BUTTON_ENTWARE_SUBMIT\"]').show();
		$('[sid=\"S_BUTTON_ENTWARE_INSTALL\"]').hide();
		$('[sid=\"L_ENTWARE_STATUS\"]').text(M_lang['S_ENTWARE_STATUS_NONE']);
	}else{
		switch(status_data.entware.install_stat){
			case 'READY':
				if(selectedval == config_data.entware.datafolder){
					$('[sid=\"L_TELNET_ONOFF_LINE\"]').show();
					$('[sid=\"S_BUTTON_ENTWARE_SUBMIT\"]').show();
					$('[sid=\"S_BUTTON_ENTWARE_INSTALL\"]').hide();
				}else{
					$('[sid=\"L_TELNET_ONOFF_LINE\"]').hide();
...
function InsertEntwareSpecialOption()
{
	$('[sid="C_ENTWARE_DATAFOLDER"] option[value=""]').after("<option value='clear'>" + M_lang['S_ENTWARE_NO_SELECT'] + "</option>");
}

iux_update_local_func['entware'] = function(id, bvalue)
{
	switch(id)
	{
		case 'C':
			insert_hddlist("C_ENTWARE_DATAFOLDER");
			InsertEntwareSpecialOption();
			status_control_visible(false, "L_ENTWARE_DATAFOLDER" );
			insertDefaultOption( 'entware' );
			UpdateEntwareUI();
			status_control_enable(true, "S_BUTTON_ENTWARE_SUBMIT");
			status_control_enable(true, "S_BUTTON_ENTWARE_INSTALL");
			break;
		case 'D':
			UpdateEntwareUI();
			if(status_data.entware.install_stat == 'READY'){
				if(status_data.entware.run_stat == 'STARTED_STATUS')
					$('[sid=\"D_ENTWARE_STATUS\"]').text(M_lang['S_ENTWARE_EXTSTATUS_INSTALLED'] + ' ' + M_lang['S_ENTWARE_EXTSTATUS_RUN']);
				else
					$('[sid=\"D_ENTWARE_STATUS\"]').text(M_lang['S_ENTWARE_EXTSTATUS_INSTALLED'] + ' ' + M_lang['S_ENTWARE_EXTSTATUS_STOP']);
			}else{
				$('[sid=\"D_ENTWARE_STATUS\"]').text(M_lang['S_ENTWARE_EXTSTATUS_NOT_INSTALLED'] + ' ' + M_lang['S_ENTWARE_EXTSTATUS_STOP']);
			}
			break;
	}
};

add_listener_local_func['entware'] = function(addoption, bvalue)
{
	$('[sid=\"C_ENTWARE_DATAFOLDER\"]').unbind().change(function() {
		UpdateEntwareUI();
		$(this).selectmenu('refresh', true);
	});
	$('[sid=\"C_ENTWARE_TELNET\"]').unbind();
	$('[sid=\"S_BUTTON_ENTWARE_INSTALL\"]').unbind().click(function() { 
		var localpostdata = [];
		localpostdata.push({name : "install_entware", value : "install"});
		submit_local('entware',localpostdata); 
	});
	$('[sid=\"S_BUTTON_ENTWARE_SUBMIT\"]').unbind().click(function() { 
		submit_local('entware'); 
	});
};

submit_local_func['entware'] = function(localpostdata)
{
	if(CheckEntwareFolder() && !check_input_error("C_ENTWARE_DATAFOLDER") && !check_input_error("L_ENTWARE_DATAFOLDER", regExp_kor))
	{
		insert_postdata_datafolder("L_ENTWARE_DATAFOLDER");
		if(localpostdata){
			localpostdata.push({name : "datafolder", value : $('[sid=\"C_ENTWARE_DATAFOLDER\"] option:selected').val()});
		}
		return true;
	}
	return false;
...
function result_config()
{
	...
	if( config_data.entware )
		config_data.entware.select = config_data.mount;
...
function load_userlist(userlist)
{
	$("#userlist").on('click', function(){
		$("#right_panel").css("background-color", "#FFFFFF");
...
{
	$("#right_panel").css("background-color", "#F6F6F6");
...
function iux_update_local(id)
{
	...
	else if(id == 'D')
	{
		...
		if( config_data.entware )
			iux_update_local_func['entware'](id);
 	}
	else if(id == 'S')
	{
	}
...
function status_control_checkbox(bvalue, sid)
{
	...
	{
		var defaultport = config_data[s_sid[VAL_ARTICLE].toLowerCase()].default_port || eval(ctrl_sid);
...
function ctrlmgr_defaultvalue(sid)
{
	...
	$('[sid="'+ctrl_sid+'"]').change(function(){
		...
		status_control_enable(bvalue, sid);
		if(bvalue == 0)
			$('[sid="'+sid+'"]').val( config_data[s_sid[VAL_ARTICLE].toLowerCase()].default_port || eval(ctrl_sid) );
		else
			$('[sid="'+sid+'"]').val( config_data[s_sid[VAL_ARTICLE].toLowerCase()][s_sid[VAL_TAGNAME].toLowerCase()] );
	});
}
