...
function macsearch_runfunc()
{
        ...
        $("li>a:odd").css("background-color","#F9FAF5"); //F9FAF5
...
function background_color(selector)
{
	...
	$(selector + ":odd").each(function() {
		$(this).css("background-color","#F9FAF5"); //F9FAF5
	});
}
...
iux_update_local_func['dmztwinip'] = function(identifier, viewing)
{
	if(identifier == 'C')
	{
		if(viewing)
		{
			...
			$('#ipaddr_text').text("(" + config_data.dmztwinip.client_ip + ")");

			var ipaddr;

			ipaddr = convert_ipaddr(config_data.dmztwinip.dmzipaddr);
			if(ipaddr.length !== 4)
				ipaddr = convert_ipaddr(config_data.dmztwinip.local_ip);
			...
			else if(config_data.dmztwinip.w1mode == 1)
			{
				...
				var dmz_ip = convert_ipaddr(config_data.dmztwinip.dmzipaddr);
				if( dmz_ip.length === 4 )
					$('#lc_inner_ip [sid="VALUE3"] ').val(dmz_ip[3]);
			}
...
add_listener_local_func['dmztwinip'] = function()
{
	...
	$('#current_ip_check').change(function(){
		var convert_ip;
		convert_ip = convert_ipaddr(config_data.dmztwinip.client_ip);
