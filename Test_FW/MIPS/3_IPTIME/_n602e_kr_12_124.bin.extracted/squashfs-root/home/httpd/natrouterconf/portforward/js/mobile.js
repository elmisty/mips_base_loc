...
function basic_control_event_add(ruletype, idval)
{
	...
	$('[sid="SHORTCUTS_NEWRULE"]').click(function(){
		$('#loading').popup('open');
...
{
	var HTMLStr = ('<div class=\"'+((bgidx % 2 == 1) ? 'lc_whitebox_div' : 'lc_whitebox_div')+'\" id=\"'+ruletype+'_'+ruleidx+'\" ');
	if(fixed && fixed=='1'){
	...
	HTMLStr	+= ('<div class=\"lc_rightbox_div\"><div class=\"lc_line_div '+((disabled == '1')?'lc_bg_disabledcolor1':'lc_bg_rightcolor1')+'\">');
	HTMLStr += ('<p sid=\"IP_FIELD\" class=\"'+((disabled == '1')?'lc_disabled_text':'lc_bgfont_text')+'\"></p></div>');
	HTMLStr	+= ('<div class=\"lc_line_div '+((disabled == '1')?'lc_bg_disabledcolor2':'lc_bg_rightcolor2')+'\">');
...
function update_porttrigger_status_line(idval, configObj)
{
	var valtxt = configObj.name;
	$('#'+idval+' [sid="RULENAME_FIELD"]').text(valtxt);
	console.log(configObj.triggerprotocol.toUpperCase());
...
iux_update_local_func['all'] = function(identifier)
{
	...
	if(config_data.pf_rules.length == 1 && (((config_data.pt_rules) && config_data.pt_rules.length == 1) || (!config_data.pt_rules))){locking_obj('PORTFORWARD_BACKUP', 'disabled');}
...
iux_update_local_func['setup'] = function(identifier)
{
	if(identifier == 'C')
	{
		...
		$('[sid="S_NEWRULE_BTN"]').parent().remove();
		$('[sid="S_DELETE_BTN"]').parent().remove();
...
{
	if(_idvalue.includes('forwardrule') || _idvalue.includes('triggerrule'))
                $('#right_panel').css("background-color" , "#EDF0E5");
	else
        	$('#right_panel').css("background-color" , "#F6F6F6");
