...
function add_mode_change_event()
{
	...
	{
		var this_ifinfo = config_data.linksetup[$(this).attr('name').replace("speed","wan")] || config_data.linksetup[$(this).attr('name').replace("speed","lan")];
		if(this_ifinfo.supported.indexOf($(this).val() + "HD") < 0)
		{
...
function update_linkstatus_p(target, data)
{
	...
	if(speed == "2.5G" || speed == "2500")
		speed = "2.5Gbps";
	else if(speed == "1G" || speed == "1000")
		speed = "1Gbps";
	else if(speed == "--")
		speed = "---";
	if(duplex == "--")
...
function update_linkstatus_row_config(index)
{
	...
	if(data.mode == "fixed"){
		target.find("p").eq(0).text("LAN"+port_number+M_lang["S_TABLE_TEXT5"]);
		//target.find("p").eq(1).text();
	}
	if(data.mode == "fixed")
	{
	...
	if(data.supported.indexOf("2500FD") < 0)
		target.find("select").eq(1).find('option[value="2500"]').remove();
	if(data.supported.indexOf("1000FD") < 0)
		target.find("select").eq(1).find('option[value="1000"]').remove();
	if(data.supported.indexOf("100FD") < 0)
		target.find("select").eq(1).find('option[value="100"]').remove();
	if(data.supported.indexOf("10FD") < 0)
		target.find("select").eq(1).find('option[value="10"]').remove();
...
function loadLocalPage()
{
	...
	$(".ui-page").css("background-color", "#F6F7EE");
	loadHeader();
...
function loadHeader()
{
	...
	var text="",title="";
	for(var i=1; i<=4; i++){
		title='S_TABLE1_COLUMN'+i;	
       		text +="<div class=\"lc_column"+i+"_div lc_cell_div1\">";
        	text +="<p sid=\"S_TABLE1_COLUMN"+i+"\">"+M_lang[title]+"</p></div>";
	}

        $(".cc_leftheader_div").css("width", "100%");
        $(".cc_leftheader_div").html(text);
}
