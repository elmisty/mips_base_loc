...
{
	$('[sid = "S_QOSRULE_TITLE2"]').on("click", function(){
		...
		listIndex = "";
		unlock_object('.lc_ipaddr2_div .lc_right [sid="VALUE0"]', "readonly");
		unlock_object('.lc_ipaddr2_div .lc_right [sid="VALUE1"]', "readonly");
		unlock_object('.lc_ipaddr2_div .lc_right [sid="VALUE2"]', "readonly");
		...
		//add
		$('.rpanel_div').css("display", "none");
		$('.lc_delbtn_line').remove();
		$('[sid="S_MODIFY_BUTTON"]').parent().removeClass('ui-state-disabled');
		$('[sid="S_MODIFY_BUTTON"]').attr("disabled", false);
		$('[sid="S_MODIFY_BUTTON"]').find('span').text(M_lang["S_BUTTON_SUBMIT1"]);
...
function load_qoslist_data()
{
	...
	if(rule_data.ipfrom == "Twin IP" )
	{
		...
		lock_object('input[sid = "VALUE0"]', "readonly");
		lock_object('input[sid = "VALUE1"]', "readonly");
		lock_object('input[sid = "VALUE2"]', "readonly");
...
function update_qosrules_div()
{
	...
	for(var list_name in config_data.userrule)
	{
		if(list_name == "")
			continue;
		rule_object = config_data.userrule[list_name];
		ip_name = rule_object.ipfrom;
		if(ip_name == "")
...
add_listener_local_func['qosonoff'] = function()
{
	sliderButtonEvent({ object: $('[sid="C_QOSONOFF_TCMODE"]'), runFunc: function() 
		...
		if(!check_change_value())
			return;
		if(config_data.qosrule.need_reboot && 
			(config_data.internettype.w1down > 0 || config_data.internettype.w1up > 0))
		{
			events.confirm({ msg: M_lang['S_QOSRULE_ALERT12'], runFunc: function( flag ) {
				if(flag)
					qos_reboot_submit('qosonoff');
			}});
		}
		else
			submit_local('qosonoff');
...
add_listener_local_func['internettype'] = function()
{
		...
		if(config_data.qosrule.need_reboot && config_data.qosonoff.tcmode == "1")
		{
			events.confirm({ msg: M_lang['S_QOSRULE_ALERT12'], runFunc: function( flag ) {
				if(flag)
					qos_reboot_submit('internettype');
			}});
		}
		else
...
//	$("[sid=\"MAINLISTVIEW\"] .ui-last-child").removeClass('ui-last-child').css('border-bottom','1px #ccc solid');
	...
	$(".ui-page").css("background-color", "#F9FAF5");
}
...
function result_config()
{
	...
	for( var index in config_data.userrule )
	{
		...
		{
			if(config_data.userrule[index].down % 1024 == 0)
				config_data.userrule[index].down = config_data.userrule[index].down / 1024;
			else
				config_data.userrule[index].down = (config_data.userrule[index].down / 1024).toFixed(1);
		}
		if( config_data.userrule[index].unitup == "0" )
		{
			if(config_data.userrule[index].up % 1024 == 0)
				config_data.userrule[index].up = config_data.userrule[index].up / 1024;
			else
				config_data.userrule[index].up = (config_data.userrule[index].up / 1024).toFixed(1);
		}
	}
	iux_update_local("C");
}
...
function qos_reboot_submit(service_name)
{
	submit_local(service_name);

	$("#loading").popup("close");
	reboot_timer(config_data.qosrule.rebootsec);
	stopstatus = true;
...
{
	if(_aname.includes("qosrule_"))
		$("#right_panel").css("background-color", "#EEEFE5");
	else
		$("#right_panel").css("background-color", "#F6F6F6");
