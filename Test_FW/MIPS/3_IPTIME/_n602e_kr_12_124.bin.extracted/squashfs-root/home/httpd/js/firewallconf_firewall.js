...
function All_Enable_FWForm(doc)
{
	...
	}
	{
		...
		EnableObj_For_FW(F.extaddr4);	EnableObj_For_FW(F.extaddr5);	EnableObj_For_FW(F.extaddr6);
		EnableObj_For_FW(F.extaddr7);	EnableObj_For_FW(F.extaddr8);
	}
...
function All_Disable_FWForm(doc)
{
	...
	}
	{
		...
		DisableObj_For_FW(F.extaddr4);	DisableObj_For_FW(F.extaddr5);	DisableObj_For_FW(F.extaddr6);
		DisableObj_For_FW(F.extaddr7);	DisableObj_For_FW(F.extaddr8);
	}
...
function Clear_ExternalIPBox(doc)
{
	...
	EnableObj_For_FW(F.extaddr6);
	EnableObj_For_FW(F.extaddr7);
	EnableObj_For_FW(F.extaddr8);

	F.extaddr1.value = '';	F.extaddr2.value = '';
	F.extaddr3.value = '';	F.extaddr4.value = '';
	F.extaddr5.value = '';	F.extaddr6.value = '';
	F.extaddr7.value = '';	F.extaddr8.value = '';
}
...
function onChangeExternalType(frm_doc)
{
	...
	if(F.external_type.value == 'ip'){
		...
		EnableObj_For_FW(F.extaddr5);	EnableObj_For_FW(F.extaddr6);
		EnableObj_For_FW(F.extaddr7);	EnableObj_For_FW(F.extaddr8);
	}
		...
		DisableObj_For_FW(F.extaddr5);	DisableObj_For_FW(F.extaddr6);
		DisableObj_For_FW(F.extaddr7);	DisableObj_For_FW(F.extaddr8);
	}
}
			...
			sport, eport, direction, protocol, policy, days, stime, etime, disabled, priority, wireless)
{
	...
	if(mode == 'internet'){
		...
		if(dest_addr_type == 'ip'){
			...
			if(dest_end != ''){
				F.extaddr5.value = GetIpMacNumber('ip',dest_end, 0);
				F.extaddr6.value = GetIpMacNumber('ip',dest_end, 1);
				F.extaddr7.value = GetIpMacNumber('ip',dest_end, 2);
				F.extaddr8.value = GetIpMacNumber('ip',dest_end, 3);
	...
	for(var i = 0; i < element_list.length ; i++){
		...
		if(i > 3){
			...
			if(frm_name == 'ipaddr'){
				...
				if(ipstrs[i] > ipstrs[i-4]){
					upper_netting = true;
				}
				if((!upper_netting) && (ipstrs[i] < ipstrs[i-4])){
                       			alert(FIREWALL_EXTIPADDR_UNVALID);
					element_list[i].focus();	element_list[i].select();
                        	      		return false;
				}
			}
...
function onApplyBtnClicked(gw_addr, netmask, max_count)
{
	...
	if(F.formselect.value == 'internet')
	{
		...
		if(F.external_type.value == 'ip'){
			...
			if(F.extaddr5.value != '' && F.extaddr6.value != '' && F.extaddr7.value != '' && F.extaddr8.value != '')
				F2.dest_end.value = make_ipaddr(F.extaddr5.value, F.extaddr6.value, F.extaddr7.value, F.extaddr8.value);
			else
				F2.dest_end.value = '';

			if(F2.dest_end.value == '' && (F.extaddr5.value != '' || F.extaddr6.value != '' || F.extaddr7.value != '' || F.extaddr8.value != '')){
				alert(FIREWALL_EXTIPADDR_UNVALID);	return;
			}
			if(F2.dest_end.value == F2.dest_start.value){
				F.extaddr8.focus();     F.extaddr8.select();	 alert(FIREWALL_EXTIPADDR_UNVALID);	return;
			}
		}
		else{
			F2.dest_start.value = '';
