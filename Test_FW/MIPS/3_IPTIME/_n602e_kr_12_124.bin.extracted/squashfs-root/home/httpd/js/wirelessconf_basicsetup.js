...
function wl_tag_to_full_ghz_text(wl_tag)
{
	...
	if(wl_tag == '2g')		return '2.4 GHz';
		...
		if(parent.use_triband)	return '5 GHz(1)';
		else			return '5 GHz';
	}
	else if(wl_tag == '5g2')	return '5 GHz(2)';
...
function update_channelwidth_by_mode(wl_mode)
{
	...
	if(!F.channelwidth || !F2.channelwidth)	return;
	if(!wl_mode)	wl_mode = F2.wlmodetxt.value;

	var doc = parent.document;
	var val = F.wirelessmode.value;
	var txt = '';
	var del_chanwidth = false;
	var i = 0;
	var j = 0;

	if(doc['options'+wl_mode]){
		for(i = 0; i < doc['options'+wl_mode].length; i++){
			if(doc['options'+wl_mode][i].value == val){	txt = doc['options'+wl_mode][i].text;	break;}
		}
		if(wl_mode == '5g' && (txt.indexOf('ac') == -1)){	del_chanwidth = true;}
		if(wl_mode == '5g2' && (txt.indexOf('ac') == -1)){	del_chanwidth = true;}
		if(wl_mode == '2g' && (txt.indexOf('N') == -1)){	del_chanwidth = true;}

		/*AX must be show all channel width*/
		if((txt.indexOf('ax') != -1)){del_chanwidth = false;}
	}

	for(i = 0; i < F.channelwidth.options.length; i++)	F.channelwidth.options[i] = null;
	F.channelwidth.options.length = 0;

	val = F2.channelwidth.value;

	if(doc['chanwidth'+wl_mode]){
		for(i = 0, j = 0; i < doc['chanwidth'+wl_mode].length; i++, j++){
			txt = doc['chanwidth'+wl_mode][i].text;
			if(wl_mode == '5g' && del_chanwidth && (txt.indexOf('80') != -1)){
				if(val == doc['chanwidth'+wl_mode][i].value){
					val = doc['chanwidth'+wl_mode][(((i+1) < doc['chanwidth'+wl_mode].length)?(i+1):0)].value;
				}
				j -= 1;	continue;
			}
			if(wl_mode == '5g2' && del_chanwidth && (txt.indexOf('80') != -1)){
				if(val == doc['chanwidth'+wl_mode][i].value){
					val = doc['chanwidth'+wl_mode][(((i+1) < doc['chanwidth'+wl_mode].length)?(i+1):0)].value;
				}
				j -= 1;	continue;
			}
			if(wl_mode == '2g' && del_chanwidth && (txt.indexOf('40') != -1)){	
				if(val == doc['chanwidth'+wl_mode][i].value){
					val = doc['chanwidth'+wl_mode][(((i+1) < doc['chanwidth'+wl_mode].length)?(i+1):0)].value;
				}
				j -= 1;	continue;
			}
			F.channelwidth.options[j] = new Option(doc['chanwidth'+wl_mode][i].text, doc['chanwidth'+wl_mode][i].value);
		}
	}

	F.channelwidth.value = val;
}
...
function change_onoff_val_band(doc, obj,band)
{
	if(!doc || !obj)	return;
...
function set_authline_by_select(doc, val)
{
	var tmp;
	if(val == 'nouse'){
		tmp = doc.getElementsByName('WEPSET');
		if(tmp && tmp.length != 0){for(var tp = 0; tp < tmp.length; tp ++){tmp[tp].style.display = 'none';}}
		else{
		...
		tmp = doc.getElementsByName('WEPSET');
		if(tmp && tmp.length != 0){for(var tp = 0; tp < tmp.length; tp ++){tmp[tp].style.display = '';}}
		else{
			tmp = doc.getElementById('WEPALERT');
			tmp.style.display = '';
		}
...
{
	if(!frm || !frm.radiusserver1)	return '';
...
function check_values_error()
{
	...
	if(val){
		...
		val = F2.personallist.value;
			...
			if(F2.weppass){
				if(F2.weppass.style.display == 'none')
					val = F2.weppass_text.value;
				else
					val = F2.weppass.value;
				if(val == ''){
					alert(WIRELESSCONF_BASICSETUP_WEP_KEYERR);      return true;
				}
	...
	if(F1.sidx.value == '0' && F1.country_reboot){
		if(F1.country.value != F3.country.value){
			if(!confirm(WIRELESSCONF_COUNTRY_REBOOT))	return true;
		}
	}

	if(F1.rwarning){
		if(F1.sidx.value == '0'){
			if(F1.lpchannel && F1.lpchannel.value){
				if(is_band_steering_mode(this, F1.wlmode.value, 'mode')){
					for(var i = 0; i < this.support_wl_tags.length; i++){
						if(!F2['channel_'+i])	continue;
						var ctlchan = F2['channel_'+i].value.split('.')[0];
						var lplist = F1.lpchannel.value;
						var bwidth = (F2.realchanwidth)?F2.realchanwidth.value:0;
						if(is_regulation_chan(lplist, ctlchan, bwidth)){
							if(!confirm(WIRELESSCONF_BASICSETUP_REGULATION_WARNING))	return true;
						}
					}
				}else{
					var ctlchan = F2.channel.value.split('.')[0];
					var lplist = F1.lpchannel.value;
					var bwidth = (F3.channelwidth)?F3.channelwidth.value:0;
					if(is_regulation_chan(lplist, ctlchan, bwidth)){
						if(!confirm(WIRELESSCONF_BASICSETUP_REGULATION_WARNING))	return true;
					}
				}
		...
		if(F1.sidx.value == '0'){
			if(is_band_steering_mode(this, F1.wlmode.value, 'mode')){
				for(var i = 0; i < this.support_wl_tags.length; i++){
					if(!F2['channel_'+i])	continue;
					var ctlchan = parseInt(F2['channel_'+i].value.split('.')[0]);
					if(F3.channelwidth && F3.channelwidth.value == 160) {
						if(!confirm(WIRELESSCONF_BASICSETUP_DFS_WARNING_160))   return true;
					} else if(ctlchan >= 52 && ctlchan <= 144) {
	...
	if(F1.bndrun){
		if(F1.sidx.value == '0' && (F3.bndrun.value != F1.bndrun.value) && F3.bndrun.value == '1'){
			if(!confirm(WIRELESSCONF_BANDSTEERING_WARNING))	return true;
		}
	}
...
function check_channel_value(wl_mode, country, bw, wm, ctl, cnt)
{
	var wtag = '';
	if(!window.support_wl_tags)	return '0.0';
	for(var i = 0; i < window.support_wl_tags.length; i++){
		if(parseInt(wl_mode) == parseInt(window.support_wl_tags[i].value))
			wtag = window.support_wl_tags[i].tag;
	}

	var ctl_arr = window['control_channel_arr'+wtag][country + '_' + bw];
        var cnt_arr = window['central_channel_arr'+wtag][country + '_' + bw];
...
function submit_all_data()
{
	...
	if(F2.useenterprise && F2.useenterprise.checked){
		if(F1.useenterprise)	F1.useenterprise.value = '1';
		...
		if(F1.useenterprise)	F1.useenterprise.value = '0';
		F1.personallist.value = F2.personallist.value;
		if(F1.personallist.value == 'nouse'){
			F1.wpapsk.value = '';
		}
                else if(F1.personallist.value == 'auto_wep' || F1.personallist.value == 'open_wep' || F1.personallist.value == 'key_wep'){
			if(F2.weppass){
				if(F2.weppass.style.display == 'none')
					F1.wepkey.value = F2.weppass_text.value;
				else
					F1.wepkey.value = F2.weppass.value;
			}
		}
		else{
			if(F2.password.style.display == 'none')
	...
	if(F1.country_reboot){
		if(F3.country.value != F1.country.value)	F1.country_reboot.value = 1;
	}

	if(F1.country)		F1.country.value = F3.country.value;

	if(F1.wpsnoti)		F1.wpsnoti.value = F3.wpsnoti.value;
	if(F1.bndrun)		F1.bndrun.value = F3.bndrun.value;
	...
	if(F1.vht24g)		F1.vht24g.value = F3.vht24g.value;
	if(F1.mimoant && F3.mimoant)		F1.mimoant.value = F3.mimoant.value;
	if(F1.country && F1.channelwidth && F1.wirelessmode && F1.ch_no_change){
		var channel_change_val = check_channel_value(F1.wlmode.value, F1.country.value, F1.channelwidth.value, F1.wirelessmode.value
			, F1.ctlchannel.value, F1.cntchannel.value);
		if(channel_change_val != '0.0'){
			F1.ch_no_change.value = 1;
			F1.ctlchannel.value = channel_change_val.split('.')[0];
			F1.cntchannel.value = channel_change_val.split('.')[1];
		}
	}
	}
...
function update_channel_options(country, bw, val, isauto, wl_mode, fm)
{
	...
	for(var i in ctl_arr){
		...
		if(bw == '40' && ((ctl_ch == ctl_arr[i]) && (cnt_ch == cnt_arr[i])))	is_correct = true;
		else if(bw != '40' && (ctl_ch == ctl_arr[i])){val = (ctl_arr[i] + '.' + cnt_arr[i]); is_correct = true;}
	}
	if(!is_correct){
		for(var i in ctl_arr){
...
function AllEnableBasicsetup(F)
{
	...
	EnableObj_V2(F.weppass_text); 	EnableObj_V2(F.weppass);		EnableObj_V2(F.wepview);
...
function AllDisableBasicsetup(F)
{
	...
	DisableObj_V2(F.weppass_text); 	DisableObj_V2(F.weppass);		DisableObj_V2(F.wepview);
...
function update_basicsetup_lines(wl_mode)
{
	...
	set_authline_by_select(idoc, F.personallist.value);
	...
	if(F.weppass){
		F.weppass.value = F2.wepkey.value;
		F.weppass_text.value = F2.wepkey.value;
	}
...
function update_extendsetup_lines(channelupdate)
{
	...
	update_channelwidth_by_mode(wl_mode);
	...
	if(F2.bndrun){
		F.bndrun.value = F2.bndrun.value;
		change_onoff_val(idoc, idoc.getElementById('bndrun_id'), true);
	}
	if(F2.ldpc){
	...
	if(F2.vht24g){
		if(wl_mode == '2g')
		{
			F.vht24g.value = F2.vht24g.value;
			change_onoff_val(idoc, idoc.getElementById('vht24g_id'), true);
		}
		else
		{
			F.vht24g.value = '0';
...
function wireless_reboot_process(rebootDuration)
{
	if(rebootDuration > 0){
		if(document.getElementById('reboot_seconds')){
			document.getElementById('reboot_seconds').innerHTML = rebootDuration + ' ' + MSG_REBOOT_SECONDS_REMAINS2;
		}else{
			UnMaskIt(document, 'reboot_mask');
			return;
		}
		setTimeout(function(){wireless_reboot_process(rebootDuration-1);}, 1000);
	}else{
		UnMaskIt(document, 'reboot_mask');
		location.reload();
		return;
...
function reboot_masking(rebootDuration)
{
	UnMaskIt(document, 'apply_mask');
	MaskIt(document, 'reboot_mask');

	wireless_reboot_process(rebootDuration);
}
