...
function check_dns_dynamic()
{
    ...
    if (!F.dns_dynamic_chk || F.dns_dynamic_chk.checked == false )
    {
...
function check_dns_pppoe()
{
    ...
    if (!F.dns_pppoe_chk || F.dns_pppoe_chk.checked == false )
    {
...
function update_addrs(doc, pdoc)
{
	...
	var fdnsaddr = F.fdns.value;
	var sdnsaddr = F.sdns.value;
	var tmpobj;
	...
	if(fdnsaddr != '')	fdnsaddr = fdnsaddr.split('.');
	else			fdnsaddr = ['','','',''];
	if(sdnsaddr != '')	sdnsaddr = sdnsaddr.split('.');
	else			sdnsaddr = ['','','',''];
	...
	for(var i = 1; i <= 4; i ++){
		...
		if(tmpobj = pdoc.getElementsByName('fdns_dynamic'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = fdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('fdns_pppoe'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = fdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('fdns_static'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = fdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('fdns_pptp'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = fdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('sdns_dynamic'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = sdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('sdns_pppoe'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = sdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('sdns_static'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = sdnsaddr[i-1];
		}
		if(tmpobj = pdoc.getElementsByName('sdns_pptp'+i)[0]){
			if(tmpobj.disabled)	tmpobj.value = sdnsaddr[i-1];
		}
	}
}
