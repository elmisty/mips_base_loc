...
var ApplyMBridge = (function()
{
	...
	return function(F, act, wireless_mode)
	{
		...
		if(F.ssid.value != F.tmp_ssid.value)
			F2.ssid_hex.value = "";
		else
			F2.ssid_hex.value = F.ssid_hex.value;
...
var ClickAP = (function()
{
	...
	function ModifyAP(doc,F,ap_idx)
	{
		...
		if( obj[ap_idx] && F.ssid )
		{
			...
			F.tmp_ssid.value = obj[ap_idx].value;
		}
		...
		obj = document.getElementsByName('ssid_hex');
		if( obj[ap_idx] && F.ssid_hex )
			F.ssid_hex.value = obj[ap_idx].value;
	}
