...
function local_fileform_submit(service_name, localPostdata)
{
	...
	if(localPostdata == '0')
		...
		postData = new FormData($('#iux_file_form')[0]);
...
add_listener_local_func['hubapmode'] = function()
{
	...
	$('[sid=\"S_HUBAPMODE_LINK\"]').click(function(){
...
iux_update_local_func['fan'] = function(identifier, viewing)
{
	...
	if(identifier == 'C'){
		...
		printData += (M_lang['S_FAN_SPEEDTXT'] + M_lang['S_FAN_'+status_data.fan.status.toUpperCase()]);
	}
	else{}
...
$(document).ready(function() {
	...
	$("li>a:odd").css("background-color", "#FFFFFF");
});
