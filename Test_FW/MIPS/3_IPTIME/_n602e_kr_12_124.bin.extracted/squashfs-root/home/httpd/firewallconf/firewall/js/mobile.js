...
function check_subnet_between(sip, eip)
{
	var siparr = sip.split('.');
	var eiparr = eip.split('.');
	var upper_subnet = false;

	for(var i = 0; i < siparr.length; i++){
		if(parseInt(siparr[i]) < parseInt(eiparr[i]))				upper_subnet = true;
		if((!upper_subnet) && (parseInt(siparr[i]) > parseInt(eiparr[i])))	return false;
	}
...
function make_ip_value(ruletype, selectorstr)
{
	var result = [];
	var basestr = '';
	if($('[sid=\"L_'+ruletype.toUpperCase()+'_'+selectorstr+'\"] [sid^=\"VALUE\"]').length == 8){
		...
		basestr = $('[sid=\"L_'+ruletype.toUpperCase()+'_'+selectorstr+'\"] [sid=\"VALUE4\"]').val();	basestr += '.';
		basestr += $('[sid=\"L_'+ruletype.toUpperCase()+'_'+selectorstr+'\"] [sid=\"VALUE5\"]').val();	basestr += '.';
		basestr += $('[sid=\"L_'+ruletype.toUpperCase()+'_'+selectorstr+'\"] [sid=\"VALUE6\"]').val();	basestr += '.';
		result[1] = basestr + $('[sid=\"L_'+ruletype.toUpperCase()+'_'+selectorstr+'\"] [sid=\"VALUE7\"]').val();
...
function all_enable_control(ruletype)
{
	...
	if(ruletype == 'wifirule'){locking_obj('C_'+rtype+'_BAND','readonly');}
...
function firewallrule_validate(ruletype)
{
	...
	if($('[sid="C_'+ruletype.toUpperCase()+'_SRCADDRTYPE"]').val() == 'ip'){
		...
		if(tmpresult[1] != '' && !check_subnet_between(tmpresult[0], tmpresult[1])){alert(M_lang['S_INTERNALIP_ALERT']);    return false;}
	}
	else if($('[sid="C_'+ruletype.toUpperCase()+'_SRCADDRTYPE"]').val() == 'mac'){
		var macstr = make_mac_value(ruletype);
	...
	if(mode == 'internet'){
		...
		if($('[sid="C_'+ruletype.toUpperCase()+'_DSTADDRTYPE"]').val() == 'ip'){
			...
			if(!validate_string(tmpresult[0], regExp_ip, 'match')){alert(M_lang['S_IP_INVALID']);    return false;}
			if(tmpresult[1] != '' && !validate_string(tmpresult[1], regExp_ip, 'match')){alert(M_lang['S_IP_INVALID']);    return false;}
			if(tmpresult[1] != '' && !check_subnet_between(tmpresult[0], tmpresult[1])){alert(M_lang['S_EXTERNALIP_ALERT']);    return false;}
		}
	}
	else if(mode == 'site'){
		...
		if(!validate_string($('[sid="C_'+ruletype.toUpperCase()+'_URL"]').val(), regExp_spchar, 'unpermitted')){
			//add* 0123
			$('.content_field').children(':last').css('width','23em');
...
{
	var HTMLStr = ('<div class="lc_whitebox_div" id=\"'+ruletype+'_'+ruleidx+'\" sid=\"LISTDATA\">');
...
{
	var HTMLStr = '<div class=\"lc_greenbox_div\" id=\"setup_0\" sid=\"LISTDATA\">';
...
function update_ip_field(sid, sip, eip)
{
	...
	if($('[sid=\"'+sid+'\"] [sid^=\"VALUE\"]').length == 8){for(var i = 0 ; i < 4; i ++){$('[sid=\"'+sid+'\"] [sid=\"VALUE'+(i+4)+'\"]').val(eiparr[i]);}}
	else if(eiparr[3])	$('[sid=\"'+sid+'\"] [sid=\"VALUE'+4+'\"]').val(eiparr[3]);
}
...
function get_wifi_full_ghz_text(band)
{
	if(!band)		return '';
	...
	for(var i = 0; i < support_wl_modes.length; i++){
		if(band == support_wl_modes[i].tag)	return support_wl_modes[i].text;
	}
	return '';
}
...
function update_wifi_line(idval, configObj)
{
	...
	valtxt = ('WiFi ' + get_wifi_full_ghz_text(configObj.band));
	$('#' + idval + ' [sid=\"INTIPMAC_FIELD\"]').text(valtxt);
	$('#' + idval + ' [sid=\"DAYS_FIELD\"]').text(make_line_day_string(parseInt(configObj.day), parseInt((configObj.stime != 'all')?configObj.stime:'0')
			, parseInt(configObj.etime)));
...
function fill_band_value(ruletype)
{
	...
	if(!support_wl_modes)	return;
	for(var idx = 0; idx < support_wl_modes.length; idx++){
		$('[sid=\"C_'+ruletype.toUpperCase()+'_BAND\"]')
			.append('<option value=\"'+(support_wl_modes[idx].tag)+'\">WiFi '+(support_wl_modes[idx].text)+'</option>');
	}
	$('[sid=\"C_'+ruletype.toUpperCase()+'_BAND\"]').append('<option value=\"all\">WiFi ALL</option>');
...
{
	//$('#right_panel').css('background-color' , '#EDF0E5');
...
function loadLocalPage()
{
	...
	popup = TapholdPopup();
	$(".ui-page").css('background-color', '#EEF2E5');
}
...
function iux_update_local(identifier)
{
	if(!identifier){	//list update
		...
		if(config_data && config_data.support_wl_list && !support_wl_modes){support_wl_modes = config_data.support_wl_list;}
