...
#				aes|AES) crypto="CCMP" ;;
	...
	[ "$wps_enable" == "1" ] && [ "$hide_ssid" == "0" ] && {
		...
		append "$var" "device_type=$device_type" "$N"
                [ "$vif" == "ath0" ] &&{
			append "$var" "device_name=ZyXEL NBG6616 2.4G Access Point" "$N"	
		}
		[ "$vif" == "ath10" ] &&{
			append "$var" "device_name=ZyXEL NBG6616 5G Access Point" "$N"
		}
		append "$var" "manufacturer=ZyXEL Communications Corp." "$N"
		append "$var" "wps_rf_bands=ag" "$N"
		append "$var" "model_name=NBG6616" "$N"
		append "$var" "model_number=NBG6616" "$N"
