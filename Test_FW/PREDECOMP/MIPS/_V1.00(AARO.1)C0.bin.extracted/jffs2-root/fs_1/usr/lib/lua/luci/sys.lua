...
function net.fw_service(callback)
	...
	for s in pairs(sections_or) do
		if not (s == "general" or s == "remote_www" or s == "remote_telnet" or s == "remote_https") then
...
function net.portfw(callback)
	...
	for s in pairs(sections_or) do
		if not (s == "general") then
			...
			--table.insert(sections,num,s)
			--   WenHsien-2014.0124:
			--     This function add to 9th will change some key,
			--     and add to 11th/23th will lose some pair !
			--     So I use static assignment instead.
			sections[num]=s
		end
	end
...
function net.dhcp(callback)
	local routes = { }
local index = 1;
	exec("cp /tmp/dhcp.leases /tmp/dhcp.leases.bk")
	for line in io.lines("/tmp/dhcp.leases") do

		local sNum, macAdd, ipAdd, hostName,macAdd2 = line:match(
			"(%d+) ([a-zA-F0-9:]+) ([0-9.]+) ([^%s]+) ([a-zA-F0-9:*]+)"
			-- "^%s" : All non-space characters, NBG6716, WenHsiang, 2012/05/02
		)
	...
	local ck_lease_tb=exec("cat /tmp/dhcp.leases")
	if not ck_lease_tb then 
		exec("cp /tmp/dhcp.leases.bk /tmp/dhcp.leases")
	end
	exec("rm /tmp/dhcp.leases.bk")
...
function lan_topology(callback)
	local lanDev = { }
	local lanNumber = exec("grep -rn \"\" /var/lltd_num_lan | wc -l")
	local parentalFlag, block
	
	local parental_count = uci:get("parental","general","count")
	...
	local StatusEnable, name, mac_address

	
	if lanNumber then
		local rt = {}
		for line in io.lines("/var/lltd_num_lan") do
			rt = {}
			local MAC, IP, HostName, LanType, osType = line:match(			"([a-zA-F0-9:]+)%s+([0-9-.*%s]+)%s+([a-zA-Z0-9-_#@%.^\"!&*%s]+)%s+([a-zA-Z0-9-_#@%.^\"!&*]+)%s(%s+[a-zA-Z0-9-_#@%.^\"!&*]+)"
			)
			if LanType == "Ethernet" then
				LanType = "0"
			else
				LanType = "1"
			end
			...
			if parental_enable == "1" then
				if parental_count ~= "0" then
					for i = 1,parental_count do
						local rule_enable = uci:get("parental","rule"..i,"enable")
						if rule_enable == "1" then
							local parental_mac = uci:get("parental","rule"..i,"src_mac")
							if MAC == parental_mac then
								parentalFlag = "1"
								break
							else
								parentalFlag = "0"
			...
			if firewall_enable == "1" then
				if rules_count ~= "0" then
					for i=1,rules_count do
						...
						mac_address = uci:get("firewall", "rule"..i,"mac_address")
...
function wlan_topology(callback)
	local wlanDev = { }	
	local wlanNumber = exec("grep -rn \"\" /var/lltd_num_lan | wc -l")
	local parentalFlag, block
