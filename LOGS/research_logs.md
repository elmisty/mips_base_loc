...
### `21.03.05(금)
좀 더 목적을 명확하게 하는 것이 좋겠다.
현 프로젝트의 목적..?
: 베이스 주소를 찾기 어려운 펌웨어에 대해 베이스 주소를 빠르게 찾아낼 수 있도록 돕는것.

그래서 기존에 우리는 어떻게 하는가?
1. 우선 펌웨어를 IDA에 올리고
2. 코드로 강제로 맞춘 뒤
3. CODE Seg와 DATA Seg를 나누고
4. 주소들을 쭉 순회하며 어드레스처럼 보이는 주소를 수집하고
5. 해당 주소들의 하위 2-byte를 날려 Rabase 해본다.
6. 해본 Rebase의 결과로써 문자열이 완성이 된다면 이를 Rebase가 완료되었다 판단한다.

## 4월
### `21.04.13(월)
탐지 방안 확립
- 주소의 탐지 범위를 줄이는 방안
- 메모리 권한에 따른 메모리 레이아웃을 활용, 메모리 내의 탐지 주소 범위를 줄이는 것을 그 목적으로 함.

커널 영역에서 사용되는 코드는 $k0, $k1 레지스터만을 사용
유저 영역에서 사용되는 코드는 나머지 레지스터를 사용
=> 이를 어떻게 구분할건데..?

### `21.04.29(목)
기존의 메모리 레이아웃을 활용하면 베이스 주소를 구할 수 있다..?

MIPS의 메모리를 사용하는 타입은 두 가지가 존재
